<template>
	<div>
		...
		<p class="text">订单记录</p>
		<el-card>
			<el-table :data="tableData" stripe width="100%" scope="scope">
				<el-table-column prop="id" label="编号" min-width="34" align="center" />
				<el-table-column prop="orderName" label="联系人" align="center" min-width="70" />
				<el-table-column prop="createTime" label="下单时间" align="center" />
				<el-table-column prop="orderAllPrice" label="总价" align="center" />		
			</el-table>
		</el-card>
		<div>
			<el-pagination small @current-change="handleCurrentChange" :current-page="currentPage4" :page-size="5"
				:page-count="total" layout="prev, pager, next, jumper">
			</el-pagination>
		</div>
	</div>
</template>
...
export default {
	...
	mounted() {
		this.getOrderData(this.$store.state.token)
	}
}
</script>
